"""
WAVE R TEST 4 — HOLLOWING SHORT BACKTEST REFINEMENT (20-DAY HMS WINDOW)
Y2AI Research | April 2026

Same logic as wave_r_test4.py but classifies on hms_chg20 instead of
hms_chg5 — the Hollowing Short thesis is structural, so a longer HMS
window should filter noise better.

INPUTS:
    wave_r_events_v2.csv           — events file with hms_chg20 column
                                      (run _add_hms_chg20.py first)
    hollowing_short_trades.tsv

OUTPUTS:
    wave_r_hs_refinement_hms20.csv
    wave_r_hs_summary_hms20.txt
"""
...
# ── LOAD WAVE R EVENTS (v2 with hms_chg20) ───────────────────────────────────
...
try:
    ...
    if "hms_chg20" not in events.columns:
        print("ERROR: events file missing hms_chg20 column.")
        print("Run _add_hms_chg20.py first.")
        raise SystemExit(1)
except FileNotFoundError:
    print("ERROR: wave_r_events_v2.csv not found.")
    print("Run _add_hms_chg20.py first to produce it.")
    raise SystemExit(1)

# ── RECLASSIFY ON 20-DAY HMS WINDOW ───────────────────────────────────────────
def reclassify(hms_chg):
    if pd.isna(hms_chg):
        return "UNKNOWN"
    if hms_chg > HMS_STABLE:
        return "WAVE_R_STRONG"       # DM down, HMS rising over 20d
    if abs(hms_chg) <= HMS_STABLE:
        return "WAVE_R"               # DM down, HMS flat over 20d
    if hms_chg <= -HMS_DECLINE:
        return "GENUINE_EXIT"         # both falling over 20d
    return "AMBIGUOUS"

events["classification"] = events["hms_chg20"].apply(reclassify)

print("\nClassification breakdown (20-day window):")
for cls, n in events["classification"].value_counts().items():
...
for _, trade in trades.iterrows():
    ...
    results.append({
        **trade.to_dict(),
        ...
        "hms_chg20_at_entry": signal.get("hms_chg20"),
...
for _, r in df.sort_values(["wave_r_class", "ticker"]).iterrows():
    ...
    hms_str = f"{r['hms_chg20_at_entry']:.3f}" if pd.notna(r.get("hms_chg20_at_entry")) else "N/A"
...
# ── SAVE ──────────────────────────────────────────────────────────────────────
df.to_csv("wave_r_hs_refinement_hms20.csv", index=False)

with open("wave_r_hs_summary_hms20.txt", "w", encoding="utf-8") as f:
...
print("\nSaved: wave_r_hs_refinement_hms20.csv")
print("Saved: wave_r_hs_summary_hms20.txt")
