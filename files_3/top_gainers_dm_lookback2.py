...
# Minimum price and volume to exclude penny stocks and illiquid names
MIN_PRICE   = float(os.getenv('DM_MIN_PRICE',  '10.0'))
MIN_VOLUME  = float(os.getenv('DM_MIN_VOLUME', '500000'))
...
def fetch_top_gainers(limit: int = TOP_N) -> pd.DataFrame:
    ...
    before = len(df)
    df = df[
        (df['Price'].fillna(0)      >= MIN_PRICE)  &
        (df['Day Volume'].fillna(0) >= MIN_VOLUME)
    ].reset_index(drop=True)
    df['Rank'] = range(1, len(df) + 1)
    filtered = before - len(df)
    if filtered > 0:
        print(f"[FILTER] Removed {filtered} names below ${MIN_PRICE:.0f} or "
              f"{int(MIN_VOLUME):,} volume. {len(df)} institutional names remain.")
...
def print_summary(df: pd.DataFrame):
    ...
    print(f"  Filter: Price >= ${MIN_PRICE:.0f} | Volume >= {int(MIN_VOLUME):,}")
    print("=" * 75)
...
def style_workbook(path: Path, row_count: int):
    ...
    lb_header_fill = PatternFill("solid", fgColor="2E4057")
...
def write_dated_excel(df: pd.DataFrame) -> Path:
    ...
    with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
        ...
        notes_data = pd.DataFrame({"Notes": [
            ...
            f"Filter: Price >= ${MIN_PRICE:.0f} and Volume >= {int(MIN_VOLUME):,} (penny stocks excluded).",
