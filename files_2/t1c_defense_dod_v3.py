"""
T1C DEFENSE / DOD SECTOR SIMULATION — v3 (with Sharpe)
========================================================
"""
...
CONFIG_LOCAL = {
    ...
    'UNIVERSE': [
        ...
        'BAH','SAIC','CACI','PLTR','AXON','KTOS','RCAT',
    ],
...
def main():
    print("\nT1C DEFENSE / DOD SECTOR — v3 (Sharpe enabled)")
    print("Production Spec: DM>=65, 5d MA rising, Exit MA20<50, DM-tiered sizing")
    print(f"Universe: {', '.join(CONFIG_LOCAL['UNIVERSE'])}")
    print("Period: 2016-01-01 → 2026-03-15\n")

    result = run_simulation(CONFIG_LOCAL)

    print(f"\n{'='*60}")
    print("DEFENSE / DOD RESULTS")
    print(f"{'='*60}")
    print(f"  Tickers in universe: {len(CONFIG_LOCAL['UNIVERSE'])}")
    if result.get('skipped_tickers'):
        print(f"  Skipped (no data):   {', '.join(result['skipped_tickers'])}")
    ...
    print(f"\n  ── RISK-ADJUSTED ──────────────────────────────────")
    print(f"  Sharpe Ratio:    {result['sharpe']}  (>1.0 good, >2.0 excellent)")
    print(f"  Sortino Ratio:   {result['sortino']}")
    print(f"  Calmar Ratio:    {result['calmar']}  (CAGR / |max DD|)")
    print(f"\n  YEARLY NAV:")
    for e in result['nav_log']:
        print(f"    {e['date']}: ${e['nav']:,.0f} ({e['return_pct']:+.1f}%)")
    print(f"\n  Reference: Full_Universe 28 tickers = 19.3% CAGR")
    print(f"{'='*60}")
