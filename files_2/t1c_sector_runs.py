"""
T1C SECTOR-BY-SECTOR SIMULATION
=================================
Runs production spec (Test 8) separately for each sector cluster.
Reports CAGR, max drawdown, hit rate, and trade count per sector.
...
def main():
    ...
    print(f"{'='*80}")
    print(f"{'Sector':<20} {'CAGR':>8} {'MaxDD':>8} {'Trades':>7} {'HitRate':>8} {'FinalNAV':>16}")
    print(f"{'-'*70}")
    ...
    for sector_name, results in all_results.items():
        ...
        print(f"{sector_name:<20} {cagr:>7.1f}% {max_dd:>7.1f}% {total_trades:>7} {avg_hit:>7.1f}% ${last_nav:>14,.0f}")
